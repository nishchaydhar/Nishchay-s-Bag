...
export const Roulette: React.FC<RouletteProps> = ({ quests }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-ink/5 flex flex-col items-center justify-center relative overflow-hidden h-full">
      ...
      <div className="relative mb-6 scale-90">
        {/* Pointer */}
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 z-20 text-ink drop-shadow-md">
           <ArrowDown size={32} fill="#2D2A26" />
        </div>

        {/* The Wheel */}
        <div 
          className="w-48 h-48 rounded-full shadow-lg border-4 border-white relative transition-transform duration-[3000ms] cubic-bezier(0.25, 0.1, 0.25, 1)"
      ...
      <button
        onClick={spin}
        disabled={isSpinning}
        className="bg-ink text-paper px-6 py-2 rounded-full font-sans text-sm font-medium hover:bg-ink/90 transition-all shadow-md disabled:opacity-50 active:scale-95"
      >
      ...
      {/* Result Display */}
      {result && !isSpinning && (
        <div className="mt-6 text-center animate-fade-in bg-ink/5 p-4 rounded-xl w-full">
          <p className="text-xs font-sans text-ink-light uppercase tracking-widest mb-1">Fate Chose: {result.category}</p>
          {result.quest ? (
            <div className="flex flex-col items-center">
              <p className="font-serif text-xl text-ink mb-2">"{result.quest.title}"</p>
            ...
            <p className="font-serif text-ink italic">All quests in this category are complete! Spin again.</p>
          )}
        </div>
      )}
    </div>
